        ...
        <div class="bg-white w-1/2 p-6 rounded shadow-lg">
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="mb-6">
              ...
              <input type="email" id="email" v-model="email" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="이메일 주소를 입력하세요">
            </div>
            <div class="mb-6">
              <label for="password" class="block mb-2 text-gray-700">Password</label>
              <input type="password" id="password" v-model="password" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="비밀번호를 입력하세요">
            </div>
    ...
    methods: {
      submitForm() {
        if (!this.username || !this.email || !this.password) {
          // 사용자 이름, 이메일, 비밀번호 중 하나라도 입력되지 않은 경우 모달을 띄웁니다.
